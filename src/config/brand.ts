 ...
 * Key Features: primary 컬러, 브랜드명, 로고
 ...
 * --brand-* (폴백) + --color-* (Tailwind 유틸리티) 양쪽 모두 갱신한다.
...
export function applyBrand(brand: Brand = defaultBrand): void {
  ...
  /* 시맨틱 컬러 — Tailwind 유틸리티 토큰 갱신 */
  root.style.setProperty('--color-primary', brand.primary);
  root.style.setProperty('--color-primary-hover', brand.primaryHover);
  root.style.setProperty('--color-success', brand.success);
  root.style.setProperty('--color-danger', brand.danger);
