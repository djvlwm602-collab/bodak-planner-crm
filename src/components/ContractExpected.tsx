...
import { Search, RotateCw, ChevronDown, ChevronRight, ChevronLeft, ChevronsLeft, ChevronsRight, Filter } from 'lucide-react';
    ...
    <div className="flex-1 flex flex-col h-full bg-white overflow-hidden">
      ...
      <div className="px-6 py-5 border-b border-gray-200 flex items-center justify-between shrink-0">
        <div className="flex items-end gap-4">
          <h1 className="text-[19px] font-bold text-gray-900 tracking-tight">계약 예정 고객</h1>
          <span className="text-sm text-gray-500 mb-0.5">계약 진행을 약속한 고객을 관리할 수 있습니다.</span>
        </div>
          ...
          <div className="px-6 py-4 flex items-center justify-between">
            <div className="flex items-center gap-4">
              <div className="relative">
                <select 
                  className="appearance-none bg-gray-50 border border-gray-200 text-gray-700 text-sm rounded-sm pl-3 pr-8 py-1.5 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent cursor-pointer hover:bg-gray-100 transition-colors"
                  value={selectedAgent}
                  onChange={(e) => setSelectedAgent(e.target.value)}
                >
                  <option value="전체">담당설계사 전체</option>
            ...
            <div className="flex items-center gap-2">
              ...
              <button 
                className="p-1.5 text-gray-500 hover:bg-gray-100 rounded-sm transition-colors flex items-center justify-center"
                onClick={() => {
                  setCustomerName('');
                  setSelectedAgent('전체');
                }}
              ...
              <table className="w-full text-sm text-center">
                <thead className="bg-gray-50 sticky top-0 z-10 border-b border-gray-300">
                  <tr>
                    ...
                    <th className="py-3.5 px-4 font-bold text-gray-900 w-48">계약 예정 상태 전환일</th>
                  </tr>
                </thead>
                <tbody className="divide-y divide-gray-200">
                  {mockData.map((row) => (
                    <tr key={row.no} className="hover:bg-gray-50 transition-colors">
                      ...
                      <td className="py-4 px-4 text-gray-700">{row.expectedDate}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
