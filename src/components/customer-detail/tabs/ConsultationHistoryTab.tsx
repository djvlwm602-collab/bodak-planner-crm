  ...
  return (
    <div className="flex p-6 gap-6">
      ...
      <div className="flex-1 min-w-0">
        <h2 className="text-body2 font-semibold text-text-primary mb-6">상담 이력 및 메모</h2>
          ...
          <ul className="flex flex-col gap-8">
            {mockTimeline.map((evt) => (
              <li key={evt.id} className="relative">
                ...
                {/* 제목 라인 */}
                <div className="flex items-center flex-wrap gap-2 text-body4 mb-2">
                ...
                {/* 메모 추가 버튼 */}
                <button type="button" className="inline-flex items-center gap-1 text-body5 text-text-secondary hover:text-text-primary mb-2">
                  <Plus size={13} />
                  <span>상담 메모추가</span>
                </button>
      ...
      <aside className="w-[220px] shrink-0">
        <div className="border border-border rounded-lg bg-surface p-5">
          <h3 className="text-body2 font-semibold text-text-primary mb-4">상담 현황</h3>
          <ul className="flex flex-col gap-3 text-body4">
...
function MemoInputForm() {
  return (
    <div className="mt-1 mb-3">
      <textarea
        className="w-full border border-border rounded-md px-3 py-2 text-body4 min-h-[72px] resize-none bg-surface focus:outline-none focus:border-primary"
        placeholder="상담 메모를 입력해 주세요."
      />
      <div className="flex justify-end gap-2 mt-2">
        <button type="button" className="px-3 py-1 text-body4 border border-border rounded-md text-text-secondary hover:bg-bg transition-colors">
          취소
        </button>
        <button type="button" className="px-3 py-1 text-body4 bg-primary text-white rounded-md hover:bg-primary-hover transition-colors">
          확인
        </button>
      </div>
    </div>
...
function MemoCard({ content, createdAt }: { content: string; createdAt: string }) {
  return (
    <div className="mt-2 border border-border rounded-md bg-surface px-4 py-3">
      <p className="text-body4 text-text-secondary leading-relaxed">{content}</p>
      <div className="flex items-center justify-between mt-2.5 text-body5 text-text-disabled">
        <span>{createdAt}</span>
